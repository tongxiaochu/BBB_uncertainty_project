...
from grover.util.nn_utils import index_select_ND, get_activation_function
...
class MTBlock(nn.Module):
    ...
    def forward(self, batch, features_batch=None):  # why use features_batch here?
...
class GTransEncoder(nn.Module):
    ...
    def pointwise_feed_forward_to_atom_embedding(self, emb_output, atom_fea, index, ffn_layer):
        ...
        aggregate to atom.
        """
        aggr_output = self.select_neighbor_and_aggregate(emb_output, index)
    ...
    def pointwise_feed_forward_to_bond_embedding(self, emb_output, bond_fea, a2nei, b2revb, ffn_layer):
        ...
        aggregate to bond.
        """
        aggr_output = self.select_neighbor_and_aggregate(emb_output, a2nei)
        # remove rev bond / atom
    ...
    def atom_bond_transform(self,
        ...
        :param atom_output:
        :param bond_output:
        :param original_f_atoms:
        :param original_f_bonds:
        :param a2a:
        :param a2b:
        :param b2a:
        :param b2revb:
        ...
        if to_atom:
            # atom to atom
            atom_output, _ = self.pointwise_feed_forward_to_atom_embedding(atom_output, original_f_atoms, a2a,
                                                                           self.ffn_atom_from_atom)
            # bond to atom
            bond_output, _ = self.pointwise_feed_forward_to_atom_embedding(bond_output, original_f_atoms, a2b,
                                                                           self.ffn_atom_from_bond)
            return self.atom_from_atom_sublayer(None, atom_output), self.atom_from_bond_sublayer(None, bond_output)
        else:  # to bond embeddings

            # atom to bond
            atom_list_for_bond = torch.cat([b2a.unsqueeze(dim=1), a2a[b2a]], dim=1)
            atom_output, _ = self.pointwise_feed_forward_to_bond_embedding(atom_output, original_f_bonds,
                                                                           atom_list_for_bond,
                                                                           b2a[b2revb], self.ffn_bond_from_atom)
            # bond to bond
            bond_list_for_bond = a2b[b2a]
            bond_output, _ = self.pointwise_feed_forward_to_bond_embedding(bond_output, original_f_bonds,
                                                                           bond_list_for_bond,
                                                                           b2revb, self.ffn_bond_from_bond)
            return self.bond_from_atom_sublayer(None, atom_output), self.bond_from_bond_sublayer(None, bond_output)
