...
def get_estimator(estimator='RF'):
    estimators = {
        ...
        'MLP_PCP': MLPClassifier(hidden_layer_sizes=(1500, 1000, 500),
                                 max_iter=1000,
                                 batch_size=16,
                                 random_state=10,
                                 learning_rate_init=0.0001,
        ...
        'RF_PCP': RandomForestClassifier(n_estimators=250,
                                     max_depth=20,
                                     criterion='gini',
                                     n_jobs=-1,
                                     random_state=10,
...
def run_training(val_data, estimator_name='RF'):
    ...
    if not val_data:
        ...
        save_dir = 'results/rf_ecfp_cls/model_0'
        ...
        test_data_new.to_csv(os.path.join(save_dir, 'Sdata-process-flow-step5-testdata_prediction.csv'),
                             index=False)
...
def run_training_PCP(val_data, estimator_name='RF'):
    # X_test, y_test, test_data, test_feature_dicts = load_test_data(prop='PCP')
    if not val_data:
        ...
        save_dir = 'results/mlp_pcp_cls_new_2/model_1'
        if os.path.exists(save_dir) == False:
            os.mkdir(save_dir)
        with open(os.path.join(save_dir, 'model.ckpt'), 'wb') as f:
            pickle.dump(estimator, f)
        ...
        y_pred = estimator.predict_proba(X_test)[:, 1]
        test_data_new['probability'] = y_pred
        test_data_new.to_csv(os.path.join(save_dir, 'Sdata-process-flow-step5-testdata_prediction.csv'), index=False)
        ...
        X_train, y_train, train_data, train_feature_dicts, scaler, train_data_new = load_train_data(validation=True,
                                                                                                    prop='PCP')
        X_test, y_test, test_data, test_feature_dicts, test_data_new = load_test_data(prop='PCP', scaler=scaler)
        data_report(y_train)
        # data_report(y_test)
...
def run_training_AttentiveFP(val_data, model_n=0):
    X_test, y_test, test_data, test_feature_dicts, test_data_new = load_test_data(prop='PCP', scaler=None)
    if not val_data:   #val_data = False
        X_train, y_train, train_data, train_feature_dicts, scaler, train_data_new = load_train_data(validation=False,
                                                                                                    prop='ECFP')
        train_monitors = defaultdict(list)
        val_monitors = defaultdict(list)
        ...
        X_train, y_train, train_data, train_feature_dicts = load_train_data(validation=True, prop='ECFP')
        data_report(y_train)
        data_report(y_test)
        kf = KFold(n_splits=3, shuffle=True, random_state=0)
...
if __name__ == '__main__':
    ...
    if model_name == "BBBscore":
    ...
    elif model_name in ['RF_PCP', 'MLP_PCP']:
        val_data = False  # False:M-data train,predict S-data
        ...
        best_model = run_training_AttentiveFP(val_data, model_n=seeds)
        Stest_loss, Stest_y_list, Stest_y_pred_list = predict(best_model,
                                                              test_data,
                                                              test_feature_dicts,
                                                              loss_function)
        y_pred = Stest_y_pred_list[0]
