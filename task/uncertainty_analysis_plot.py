...
def normalize_uncertainty(result_df):
    np.random.seed(123)
    result_df['Random'] = np.random.rand(result_df.shape[0])
...
if __name__ == '__main__':
    ...
    colors = {k: v for v, k in zip([c for i, c in enumerate(sns.color_palette("RdBu_r", 9)) if i not in (3, 4, 5)],
                                   ['Entropy', 'MC-dropout', 'Multi-initial', 'FPsDist', 'LatentDist', 'Ensemble'])}
    colors['Random'] = (0.5, 0.5, 0.5)
    args.colors = colors
    # figure 3
    uname = ['Entropy', 'MC-dropout', 'Multi-initial', 'FPsDist', 'LatentDist', 'Random']
    print("Draw Figure 3 saved in {}".format(os.path.join(args.save_dir, 'uncertainty_histplots_'+'_'.join(uname)+'.png')))
    draw_histplot(result_df, uname=uname, args=args)

    # figure 4A
    uname = ['Entropy', 'MC-dropout', 'Multi-initial', 'FPsDist', 'LatentDist', 'Random']
    print("Draw Figure 4A saved in {}".format(
    ...
    print("Draw Figure 4B saved in {}".format(
        os.path.join(args.save_dir, 'uncertainty_lineplots_' + '_'.join(uname) + '.png')))
    draw_lineplot(result_df, uname=uname, args=args)

    # figure 4C
    uname = ['Ensemble']
    print("Draw Figure 4C saved in {}".format(
